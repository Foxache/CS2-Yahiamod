...
def update_ping():
    ...
    # Handle sound playback based on ping group
    if current_group == 3:  # Always true now, since forced last group
        if not playing:
            play_sound_moroccan()  # Start sound only if it's not already playing
    else:
        if playing:
            stop_moroccan_sound()  # Stop sound when leaving group 4
    if prev_ping > 850:
        trigger_event()
